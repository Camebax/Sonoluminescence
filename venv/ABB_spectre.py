...
def get_ABB_power(lda_min, lda_max, T, power = 10, sum_energy = 0): # ВВОД ДЛИН ВОЛН В МЕТРАХ!!!
    ...
    while lda <= lda_max: # Перебор всех длин волн из указанного диапазона
        u_lda = (8*pi*h*c/lda**5)/(e**(h*c/(lda*k*T))-1) # Спектральная плотность энергии для данной lda
        sum_energy += (u_lda * 10**(-9)) * 2 # Прибавляем спектральную плотность в данной точке к общей мощности, тут и далее умножение на 2, тк шаг 2нм
